...
static bool PlayerHome(const GameState &state) {
...
static bool GooberHomeHome(const Goober &goober) {
	return CheckCollisionRecs({ -1, -1.5f, 2, 3 }, { goober.x - 0.4f, goober.y - 0.4f, 0.8f, 0.8f });
}

static void SpawnGoober(Goober &goober) {
...
static void ChooseBrokenTile(GameState &state) {
...
static float BrokeTileDist(const GameState &state) {
	return Dist(state.btx, state.bty, state.bot_x, state.bot_y);
}

static bool OnBrokeTile(const GameState &state) {
...
static bool DisplayDeath(const GameState &state) {
	bool f = true;
